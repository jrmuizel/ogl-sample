...
**
** $Date$ $Revision$
** $Header: //depot/main/gfx/lib/opengl/glcore/s_names.c#5 $
*/
#include "context.h"
#include <GL/gl.h>
#include <stdio.h>
...
struct __GLnamesArrayTypeInfoRec {
    __GLnamesArrayType type;	/* type of names array */
    ...
    GLboolean (*alloc)(__GLcontext *gc, size_t size);
...
static  __GLdlist emptyDlist = {
	1,			/* refcount */
	__glEmptyFreeDlist,	/* free */
	__glEmptyExecuteDlist,	/* execute */
	NULL,			/* segment */
	{0,0,0,0},		/* local[0] - local[3] */
};


/*
** Array of structures that contain the information needed to handle
** specific data types.  Note that the __GLnamesArrayType enum
** is used to index into this array, so it must be initialized in
** the order corresponding to the enum, and the enum must start at
** zero.  Also note that this is read only data designed to be
** shared between multiple threads.
*/
static __GLnamesArrayTypeInfo namesTypeInfo[] = {
    {
	__GL_NAMES_DLIST,		/* type */
	&emptyDlist,			/* ptr to empty data struct */
	sizeof(__GLdlist),		/* dataSize */
	__glDisposeDlist,		/* free callback */
	NULL,				/* alloc callback */
    },
    {
	__GL_NAMES_TEXOBJ,		/* type */
	NULL,				/* ptr to empty data struct */
	sizeof(__GLtexture),	/* dataSize */
...
*/
__GLnamesArray *__glNamesNewArray(__GLcontext *gc, __GLnamesArrayType type)
{
    ...
    array->tree = NULL;
    array->depth = 0;
    array->dataInfo = &(namesTypeInfo[type]);
    assert(array->dataInfo->type == type);
    /*
...
static void freeLeafData(__GLcontext *gc, void **dataList)
{
    /*
...
static void freeLeaf(__GLcontext *gc, __GLnamesLeaf *leaf)
{
    if (leaf->dataList) {
...
static void freeBranch(__GLcontext *gc, __GLnamesBranch *branch)
{
...
*/
void __glNamesFreeTree(__GLcontext *gc, __GLnamesArray *array,
...
void __glNamesFreeArray(__GLcontext *gc, __GLnamesArray *array)
{
    GLuint i;
...
** display list.
*/
static __GLnamesLeaf *findLeaf(__GLnamesArray *array, GLuint number, 
			       			GLint exact)
{
...
*/
static void copyLeafInfo(__GLnamesLeaf *leaf, __GLnamesLeaf *newleaf)
{
    GLint offset;
    GLuint number;
    GLuint i;
...
*/
static GLboolean fixMemoryProblem(__GLcontext *gc, __GLnamesArray *array)
{
    GLuint i;
...
*/
static GLuint computeMax(__GLnamesBranch *branch, GLint curdepth, 
			 GLint maxdepth)
{
    __GLnamesLeaf *leaf;
...
*/
static void pushMaxVal(__GLnamesBranch *child, GLuint maxval)
{
...
static GLboolean allocLeafData(__GLcontext *gc, __GLnamesLeaf *leaf)
{
    GLint number;
    GLint i;
...
static GLboolean reallocLeafData(__GLcontext *gc, __GLnamesLeaf *leaf)
{
    size_t number;
    void **answer;
...
static __GLnamesLeaf *allocLeaf(__GLcontext *gc, __GLnamesArray *array)
{
    __GLnamesLeaf *leaf;
...
*/
static __GLnamesBranch *allocBranch(__GLcontext *gc, __GLnamesArray *array)
{
    __GLnamesBranch *branch;
...
*/
static void deleteChild(__GLnamesArray *array, __GLnamesBranch *parent, 
...
*/
static void addChild(__GLnamesBranch *parent, __GLnamesBranch *child, 
...
** changes.
*/
static void splitParent(__GLnamesBranch *parent, 
...
*/
static void buildParent(__GLnamesBranch *parent, __GLnamesBranch *child1, 
...
*/
static void insertLeaf(__GLcontext *gc, __GLnamesArray *array,
				__GLnamesLeaf *leaf)
{
    __GLnamesBranch *extraChild;
    __GLnamesBranch *branch;
...
*/
static void deleteLeaf(__GLcontext *gc, __GLnamesArray *array,
				__GLnamesLeaf *leaf)
{
...
*/
static void resizeLeaf(__GLcontext *gc, __GLnamesLeaf *leaf, 
...
*/
static __GLnamesLeaf *prevLeaf(__GLnamesLeaf *leaf)
{
...
*/
static __GLnamesLeaf *firstLeaf(__GLnamesArray *array)
{
    __GLnamesBranch *branch;
    GLint maxdepth, curdepth;
...
*/
static __GLnamesLeaf *nextLeaf(__GLnamesLeaf *leaf)
{
    __GLnamesBranch *branch, *child;
    GLint reldepth;
...
*/
static GLboolean mergeLeaves(__GLcontext *gc, __GLnamesLeaf *leaf1, 
			     __GLnamesLeaf *leaf2)
{
    GLuint end;
    GLuint i;
...
*/
static void mergeLeaf(__GLcontext *gc, __GLnamesArray *array,
				__GLnamesLeaf *leaf)
{
...
GLboolean __glNamesNewData(__GLcontext *gc, __GLnamesArray *array, 
...
*/
/* ARGSUSED */
void *__glNamesLockData(__GLcontext *gc, __GLnamesArray *array, 
			GLuint name)
{
    __GLnamesLeaf *leaf;
    void *data;
...
*/
/* ARGSUSED */
void __glNamesLockDataList(__GLcontext *gc, __GLnamesArray *array, 
...
*/
void __glNamesUnlockData(__GLcontext *gc, void *data)
{
    GLint *pRefcount;
    assert(data);
...
*/
void __glNamesUnlockDataList(__GLcontext *gc, GLsizei n, 
			     void *dataList[])
{
    GLint i;
    GLint *pRefcount;
    ...
    */
	...
	if (*pRefcount == 0) {
	    ...
	    ** We are the last person to see this list alive.  Free it.
	    */
	    (*gc->imports.free)(gc, (void *)pRefcount);
	}
    }
}


GLuint __glNamesGenRange(__GLcontext *gc, __GLnamesArray *array, 
			 GLsizei range)
{
    GLuint lastUsed;
    GLuint nextUsed;
...
void __glNamesDeleteRange(__GLcontext *gc, __GLnamesArray *array, 
			  GLuint name, GLsizei range)
{
	...
	*/
	if (leaf->dataList) {
		...
		if (leaf->dataList[i - leaf->start] != empty) {
		    (*leaf->dataInfo->free)(gc, 
			(__GLdlist *)leaf->dataList[i - leaf->start]);
		    leaf->dataList[i - leaf->start] = empty;
		}
	    }
	}
...
/* ARGSUSED */
GLboolean __glNamesIsName(__GLcontext *gc, __GLnamesArray *array, 
			  GLuint name)
{
    /*
    ** If the name retrieves a leaf, it is in the current name space.
    */
    if (findLeaf(array, name, GL_TRUE)) {
	return GL_TRUE;
    } else {
	return GL_FALSE;
    }
...
*/
void __glNamesGenNames(__GLcontext *gc, __GLnamesArray *array,
...
*/
void __glNamesDeleteNames(__GLcontext *gc, __GLnamesArray *array,
