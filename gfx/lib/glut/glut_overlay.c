...
static Criterion requiredOverlayCriteria[] = {
  { LEVEL, EQ, 1 },  /* This entry gets poked in determineOverlayVisual. */
  { TRANSPARENT, EQ, 1 },
  { XPSEUDOCOLOR, EQ, 1 },
  { RGBA, EQ, 0 },
  { BUFFER_SIZE, GTE, 1 }
...
determineOverlayVisual(int *treatAsSingle, Bool *visAlloced)
{
  if (__glutDisplayString) {
    ...
    /* __glutDisplayString should be NULL except if
       ...
       means using a string instead of an integer mask
       determine the visual to use. Using the function pointer
       variable __glutDetermineVisualFromString below avoids
       linking in the code for implementing
       glutInitDisplayString (ie, glut_dstr.o) unless
       glutInitDisplayString gets called by the application. */
